/**
 * Gives a dev account a visible standing: ranked elo with a win/loss record,
 ...
 *
 * Usage:
 *   npx tsx scripts/seed-dev-standing.ts [email]
 *
...
const email = process.argv[2] ?? "oemer@dev.localhost";

const user = await db.query.users.findFirst({ where: eq(users.email, email) });
if (!user) {
  console.error(
    `No user with email ${email}. Sign in via the dev button first — ` +
...
// --- ranked standing -------------------------------------------------------
// Gold territory under the provisional thresholds (RankBadge.ts): 1842 lands
// in Gold II with headroom on the progress bar.
for (const entry of [
...
// --- match history ---------------------------------------------------------
// Five finished games, newest last so the strip's DESC ordering is exercised.
// The fixture's player[0] carries clientID "aBcD1234"; making that player the
// caller and toggling the winner tuple flips win/loss per game.
const matches = [
  {
    id: "SeedGm01",
    map: "Europe",
  ...
  { id: "SeedGm02", map: "World", mode: "Team", won: false, hoursAgo: 20 },
  {
    id: "SeedGm03",
    map: "Black Sea",
    mode: "Free For All",
  ...
  { id: "SeedGm05", map: "Baikal", mode: "Team", won: false, hoursAgo: 1 },
] as const;

for (const match of matches) {
  // The real ingest receives the record as JSON over HTTP, so BigInts (gold)
  // never reach the DB layer. Round-trip through JSON the same way.
  const record = JSON.parse(
    JSON.stringify(buildGameRecord({ gameID: match.id }), (_k, v) =>
  ...
  const result = await archiveGame(match.id, { raw: record, record });
  console.log(
    `  ${match.id} ${match.map} (${match.won ? "win" : "loss"}):`,
    result,
  );
}
