  ...
  render() {
    ...
    if (this.isLoading) {
      return html`
        <button
          id="cosmetics-input"
          class="pattern-btn m-0 p-0 w-full h-full flex cursor-pointer justify-center items-center focus:outline-none focus:ring-0 bg-lt-800 overflow-hidden"
          disabled
        >
          <span
            class="w-6 h-6 border-2 border-lt-700 border-t-lt-accent rounded-full animate-spin"
          ></span>
        </button>
      `;
    }
    ...
    return html`
      <button
        id="cosmetics-input"
        class="pattern-btn m-0 p-0 w-full h-full flex cursor-pointer justify-center items-center focus:outline-none focus:ring-0 transition-colors duration-150 bg-lt-800 border border-lt-600 hover:border-lt-accent overflow-hidden"
        title=${buttonTitle}
